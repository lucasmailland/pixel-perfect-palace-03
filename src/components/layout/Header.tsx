...
export const Header = () => {
  ...
  // Browser chrome part for desktop view
  const BrowserChrome = () => (
    <>
  ...
  const Logo = () => (
    <img
      ...
      className="aspect-[1.82] object-contain w-[131px] h-auto z-0 shrink-0 my-auto"
      alt="Logo"
    />
  );
  ...
  const MobileNavMenu = () => (
    <div className={`${mobileMenuOpen ? 'block' : 'hidden'} bg-white w-full absolute top-full left-0 z-50 shadow-lg`}>
      <div className="flex flex-col">
        <a href="#" className="px-6 py-4 border-b border-gray-200 text-[#252D35]">
          Muro
        </a>
        <a href="#" className="px-6 py-4 border-b border-gray-200 text-[#313A45]">
          Cartelera
        </a>
        <a href="#" className="px-6 py-4 border-b border-gray-200 text-[#313A45]">
          Agenda
        ...
        <a href="#" className="px-6 py-4 border-b border-gray-200 text-[#ED1C24] flex items-center">
          <img
            src="https://cdn.builder.io/api/v1/image/assets/TEMP/02ca42794345cc6eef33a9ec6e05e1955b741802?placeholderIfAbsent=true"
            className="aspect-[1] object-contain w-5 mr-2"
            alt="Culture icon"
          />
          Cultura
        </a>
  ...
  // User actions menu (right side of header)
  const UserActions = () => (
    <div className="self-stretch z-0 flex items-center gap-4 text-[#252D35] justify-center my-auto">
  ...
  const MobileMenuToggle = () => (
    <button 
      ...
      className="p-2 ml-auto block md:hidden"
      aria-label="Toggle menu"
    >
      <Menu className="w-6 h-6 text-[#252D35]" />
    </button>
  );

  return (
    <header className="shadow-[0px_1px_1px_0px_rgba(0,0,0,0.08)] w-full relative">
      <div className="bg-black w-full">
        {!isMobile && <BrowserChrome />}
      </div>
      <nav className="justify-between items-center bg-white flex min-h-24 w-full text-sm font-semibold whitespace-nowrap text-center leading-6 px-6 py-3 md:px-20 relative">
        <Logo />
